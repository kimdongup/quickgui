...
2026-09-10 추가 사용자 확인: M1 macOS ARM의 **바탕화면·재부팅·SSH 검증 완료**를 사용자가 보고했다. 이 결과는 사용자 확인으로 기록하며 이 Intel 작업에서 원격 VM을 재검증했다는 의미는 아니다. Windows 후속 검증 `f3f5c2b`와 함께 [통합 후보 검토](PLATFORM_INTEGRATION.ko.md)로 이어간다.
...
- Windows: SSH/SPICE는 기존 검증에 포함되지 않았다는 2026-09-10 사용자 확인에 따라 접속 구성·검증이 필요하다. [기존 연결 검증 대조](GUEST_VALIDATION.ko.md)에 순서를 정리했다. 초기 설정·바탕화면·입력·HTTPS·정상 종료/설치 후 부팅은 사용자 확인 완료이며, 앱 재실행·설치 ISO 없는 실행·외부 TCP는 이번 직접 검사 완료다. 기록된 화면 연결은 Cocoa다. 오디오·전체 게스트 도구는 별도 항목이다.
- macOS ARM: 바탕화면·재부팅·SSH는 2026-09-10 사용자 확인 완료. 오디오·클립보드·공유 폴더 등 추가 기능은 별도 검증 항목이다. SSH 성공을 Quickgui의 SSH 버튼 지원 또는 모든 네트워크 기능의 직접 검사로 확대하지 않는다.
