...
- 현재 및 이전에 Quickgui에서 확인한 작업 폴더의 native VM과 Quickemu의 리터럴 저장 경로를 검사한다.
  ...
  `export`, 안전한 `declare`·`typeset` 옵션, `readonly`로 지정한 리터럴 경로도 검사한다.
  `source`·`.`로 다른 파일을 읽는 설정, 실행문·명령 치환·여러 대입문, 비어 있지 않은 `extra_args`는
  추가 저장소 참조를 숨길 수 있어 삭제를 거부한다. 이 제한은 삭제 검사에 적용하며 기존 VM 실행 설정을 바꾸지 않는다.
...
구현과 M1 검증은 [M1_VALIDATION.ko.md](M1_VALIDATION.ko.md), 선언형 경로 보호 수정과 Intel 회귀는 [통합 검토](PLATFORM_INTEGRATION.ko.md)를 따른다.
