...
`1877d60` 앱 코드에서 SPICE 설정·현재 소켓·세션 구분을 Flutter에 연결했다.
메모장의 새 검증 탭에서 SPICE 입력으로 `k`, 클라이언트 종료 후 새 접속으로
`kk`를 확인했다. 앱에서 시작한 같은 VM에서도 `kkk`, 재접속 후 `kkkk`를
확인했다. 앱이 시작한 일반 spicy를 종료하고 버튼으로 다시 접속했을 때 QEMU는
계속 실행됐고 새 연결 ID와 main/display/inputs/cursor 채널이 생성됐다.

일반 GTK 창은 접근성 도구로 제어할 수 없어서 실제 픽셀·키보드·클릭 판정은
SPICE 프로토콜 검증기를 사용했다. 일반 viewer의 메뉴·클립보드·오디오 등은
이번 검증 범위에 포함하지 않았다. Unix 소켓의 TCP_NODELAY 경고가 발생했으나
화면·입력·재접속 결과는 통과했다. 빌드한 backend와 viewer는 Homebrew 의존성이
있는 개인 호스트용 설치이며 바이너리 자동 배포·업데이트 기능은 추가하지 않았다.
